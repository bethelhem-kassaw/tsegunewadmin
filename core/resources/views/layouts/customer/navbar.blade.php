<nav class="bg-transparent h-20 w-full overflow-x-hidden ">
  <div class="flex h-full flex-row justify-between items-center md:px-14  px-4 border-b border-gray-700">
    <!-- image logo -->
    ...
    <!-- menu items  -->
    <ul class="hidden lg:flex flex-row gap-7 text-primary-50 ">
      <li>
        <a href="{{ route('shop.index') }}">
          Home
        </a>
      ...
      <li class="flex  items-center gap-2 group " x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
        ...
        <div class="invisible group-hover:visible absolute z-20 top-16 left-1/2 -translate-x-1/2 rounded-md border border-secondary-default  py-5 px-10 flex items-center  bg-secondary-50    mx-auto">
          <div class="grid grid-cols-3 gap-4">
            @foreach($menus as $menu)
            <div class=" ">
              <div class="mb-4 grid grid-cols-3  gap-2">
                <!-- <img class="w-10 h-10" src="{{ asset('storage/'.$menu->photos[0]) }}" alt="" srcset=""> -->
                <a href="{{ route('shop.list', $menu->id) }}" class="text-black font-bold hover:text-secondary-default whitespace-nowrap w-fit">{{ $menu->name }}</a>
              </div>
              <ol class="">
    ...
    <ul class="flex flex-row gap-5 items-center ">
      ...
      <div id="searchArea" class="hidden serachArea absolute  top-0 left-0 py-10 flex items-center  bg-primary-700 bg-opacity-90 z-10 w-full">
      ...
      <div class="hidden lg:flex flex-row gap-5">
        ...
        @guest
        <li>
          <a class="text-primary-50 font-bold text-lg" href="/login">Login</a>
        </li>
        @endguest
      </div>
      <!-- button for menu  -->
    ...
    <div class=" border p-10 relative">
      ...
      <ul class="flex gap-5 mt-5 w-full">
        ...
        <li>
          ...
          <a class="text-secondary-default font-bold text-lg" href="/login">Login</a>
